...
import { branchCondition } from '../../helpers/branch-filter';
...
export class ReportsService {
  ...
  async salesSummary(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId);
  ...
  async productRanking(tenantId: string, from?: string, to?: string, sortBy: string = 'revenue', branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId)?.replace('branch_id', 'o.branch_id') || '';
  ...
  async categoryBreakdown(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId)?.replace('branch_id', 'o.branch_id') || '';
  ...
  async serverPerformance(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId)?.replace('branch_id', 'o.branch_id') || '';
  ...
  async hourlyPatterns(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId);
  ...
  async dayOfWeekPatterns(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId);
  ...
  async orderTypeBreakdown(tenantId: string, from?: string, to?: string, branchId?: string | null) {
    const { start, end } = this.parseDates(from, to);
    const bf = branchCondition(branchId);

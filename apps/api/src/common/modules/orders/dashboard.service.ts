...
export class DashboardService {
  ...
  /**
   * Cost & margin analysis: estimated cost of today's sales based on product recipes
   */
  async getCostAnalysis(tenantId: string) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // Get today's sold items with product cost (from recipes)
    const rows: any[] = await this.prisma.$queryRaw`
      SELECT
        p.id as product_id,

...
export class DeliveryService {
  ...
  async findAll(tenantId: string, query: DeliveryQueryDto, branchId?: string | null) {
    const { status, type, date, search, page = 1, limit = 50 } = query;
    const offset = (page - 1) * limit;

    let where = `WHERE d.tenant_id = '${tenantId}'::uuid`;
    if (branchId) where += ` AND d.branch_id = '${branchId}'::uuid`;
  ...
  async create(tenantId: string, dto: CreateDeliveryOrderDto, userId?: string, branchId?: string | null) {
    const settings = await this.getOrCreateSettings(tenantId);

    if (dto.deliveryType === 'delivery' && !settings.accepts_delivery) throw new BadRequestException('Delivery no disponible');
    if (dto.deliveryType === 'pickup' && !settings.accepts_pickup) throw new BadRequestException('Pickup no disponible');
    if (dto.deliveryType === 'delivery' && !dto.addressLine1) throw new BadRequestException('Dirección requerida para delivery');
  ...
  async updateStatus(tenantId: string, id: string, newStatus: string, extra?: { reason?: string; driverName?: string; driverPhone?: string }) {
    ...
    if (newStatus === 'delivered') await this.prisma.$queryRawUnsafe(`UPDATE orders SET status = 'completed', closed_at = NOW() WHERE id = '${delivery.order_id}'::uuid`);
    if (newStatus === 'cancelled') await this.prisma.$queryRawUnsafe(`UPDATE orders SET status = 'cancelled', closed_at = NOW() WHERE id = '${delivery.order_id}'::uuid`);
  ...
  async getDashboard(tenantId: string, date?: string, branchId?: string | null) {
  ...
  async getZones(tenantId: string, branchId?: string | null) {
    const bf = branchId ? `AND branch_id = '${branchId}'::uuid` : '';
    return this.prisma.$queryRawUnsafe(
      `SELECT * FROM delivery_zones WHERE tenant_id = '${tenantId}'::uuid ${bf} ORDER BY display_order, name`
    );
  }

  async createZone(tenantId: string, dto: CreateZoneDto, branchId?: string | null) {
  ...
  async updateZone(tenantId: string, id: string, dto: UpdateZoneDto) {
    ...
    const rows: any[] = await this.prisma.$queryRawUnsafe(`SELECT * FROM delivery_zones WHERE id = '${id}'::uuid AND tenant_id = '${tenantId}'::uuid`);
    return rows[0];
  }
  ...
  async getOrCreateSettings(tenantId: string) {
    const rows: any[] = await this.prisma.$queryRawUnsafe(`SELECT * FROM delivery_settings WHERE tenant_id = '${tenantId}'::uuid`);
    if (rows.length) return rows[0];
    const created: any[] = await this.prisma.$queryRawUnsafe(`INSERT INTO delivery_settings (tenant_id) VALUES ('${tenantId}'::uuid) RETURNING *`);
    return created[0];
  }

...
let ShiftsService = class ShiftsService {
    ...
    async getCashRegisters(tenantId, branchId) {
        const bf = (0, branch_filter_1.branchConditionFor)('cr', branchId);
        const registers = await this.prisma.$queryRawUnsafe(`
      SELECT 
    ...
    async createCashRegister(tenantId, name, branchId) {
    ...
    async updateCashRegister(tenantId, registerId, data) {
    ...
    async openShift(tenantId, userId, dto, branchId) {
    ...
    `;
        ...
        const openShifts = await this.prisma.$queryRaw `
      SELECT id FROM shifts WHERE cash_register_id = ${dto.cashRegisterId}::uuid AND tenant_id = ${tenantId}::uuid AND status = 'open'
    `;
        if (openShifts.length > 0)
            throw new common_1.BadRequestException('Ya existe un turno abierto para esta caja. Ciérrelo primero.');
        const branchVal = branchId || null;
        const shiftId = await this.prisma.$queryRaw `
    ...
    async getActiveShift(tenantId, branchId) {
        const bf = (0, branch_filter_1.branchConditionFor)('s', branchId);
    ...
    `);
        ...
        const cashPayments = payments.find((p) => p.method === 'cash')?.total || 0;
        return {
            id: shift.id,
    ...
    async closeShift(tenantId, shiftId, userId, dto) {
    ...
    `;
        if (!shifts.length)
            throw new common_1.NotFoundException('Turno no encontrado o ya está cerrado');
        const shift = shifts[0];
    ...
    `;
        const cashPayments = await this.prisma.$queryRaw `
    ...
    async getShifts(tenantId, query, branchId) {
    ...
    `);
        const countResult = await this.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int as total FROM shifts s WHERE ${where}`);

...
let DashboardService = class DashboardService {
    ...
    branchFilter(branchId) {
        return branchId
            ? client_1.Prisma.sql `AND branch_id = ${branchId}::uuid`
            : client_1.Prisma.empty;
    }
    branchFilterAlias(alias, branchId) {
        return branchId
            ? client_1.Prisma.sql `AND ${client_1.Prisma.raw(alias)}.branch_id = ${branchId}::uuid`
            : client_1.Prisma.empty;
    }
    async getTodaySummary(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const bf = this.branchFilter(branchId);
        const [orders, payments, openOrders] = await Promise.all([
      ...
      `,
        ...
        WHERE p.tenant_id = ${tenantId}::uuid
          ...
          ${branchId ? client_1.Prisma.sql `AND o.branch_id = ${branchId}::uuid` : client_1.Prisma.empty}
        GROUP BY method
        ORDER BY total DESC
      `,
            this.prisma.$queryRaw `
    ...
    async getSalesByHour(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const bf = this.branchFilter(branchId);
    ...
    async getTopProducts(tenantId, branchId = null, limit = 10) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const rows = await this.prisma.$queryRaw `
      SELECT 
    ...
    async getSalesByCategory(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const rows = await this.prisma.$queryRaw `
      SELECT 
    ...
    async getTableStats(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const tbf = branchId ? client_1.Prisma.sql `AND t.branch_id = ${branchId}::uuid` : client_1.Prisma.empty;
        const rows = await this.prisma.$queryRaw `
      SELECT 
    ...
    async getRecentOrders(tenantId, branchId = null, limit = 15) {
    ...
    async getSalesTrend(tenantId, branchId = null) {
        const bf = this.branchFilter(branchId);
        const rows = await this.prisma.$queryRaw `
      SELECT 
    ...
    async getServerStats(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    async getCostAnalysis(tenantId, branchId = null) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const rows = await this.prisma.$queryRaw `
      SELECT

...
let DeliveryService = class DeliveryService {
  ...
  ${item.quantity}, ${price}, ${lineTotal}, ${lineTax}, 0,
    ...
    async getDashboard(tenantId, date) {
    ...
    async getZones(tenantId) {
        return this.prisma.$queryRawUnsafe(`SELECT * FROM delivery_zones WHERE tenant_id = '${tenantId}'::uuid ORDER BY display_order, name`);
    }
    async createZone(tenantId, dto) {
        const rows = await this.prisma.$queryRawUnsafe(`

...
let ShiftsService = class ShiftsService {
    ...
    async getCashRegisters(tenantId) {
    ...
    async openShift(tenantId, userId, dto) {
        const registers = await this.prisma.$queryRaw `
    ...
    `;
        ...
        return {
            ...
            estimatedCash: parseFloat(shift.opening_amount) + (shift.payment_breakdown?.find((p) => p.method === 'cash')?.total || 0),
        };
    }
    ...
    async getShifts(tenantId, query) {

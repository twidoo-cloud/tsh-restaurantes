...
let InventoryService = class InventoryService {
    ...
    async getStockLevelsSimple(tenantId, filters) {
        return this.getStockLevels(tenantId, filters);
    }
    async recordMovement(tenantId, userId, data) {
    ...
    async bulkAdjustment(tenantId, userId, adjustments) {
        const results = [];
    ...
    async getMovements(tenantId, filters) {
        const conditions = [`im.tenant_id = '${tenantId}'`];
        if (filters?.productId)
            conditions.push(`im.product_id = '${filters.productId}'`);
        if (filters?.movementType)
            conditions.push(`im.movement_type = '${filters.movementType}'`);
        const where = conditions.join(' AND ');
    ...
    `);
        const countResult = await this.prisma.$queryRawUnsafe(`SELECT COUNT(*)::int as total FROM inventory_movements im WHERE ${where}`);

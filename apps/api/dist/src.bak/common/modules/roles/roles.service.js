...
let RolesService = class RolesService {
    ...
    `;
        if (existing.length)
            throw new common_1.BadRequestException('Ya existe un rol con ese nombre');
    ...
    `;
        ...
        if (roles[0].slug === 'owner' && dto.name) {
            throw new common_1.BadRequestException('No se puede renombrar el rol de Dueño');
        }
        const sets = [`updated_at = now()`];
        if (dto.name)
            sets.push(`name = '${dto.name.replace(/'/g, "''")}'`);
        if (dto.description !== undefined)
            sets.push(`description = '${(dto.description || '').replace(/'/g, "''")}'`);
        if (dto.color)
            sets.push(`color = '${dto.color}'`);
        if (dto.permissions)
            sets.push(`permissions = '${JSON.stringify(dto.permissions)}'::jsonb`);
    ...
    `;
        if (users[0].count > 0) {
            throw new common_1.BadRequestException(`No se puede eliminar: ${users[0].count} usuario(s) tienen este rol asignado`);
        }

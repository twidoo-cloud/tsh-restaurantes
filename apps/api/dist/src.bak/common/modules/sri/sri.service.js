...
const sri_xml_1 = require("./sri-xml");
const sri_nota_credito_1 = require("./sri-nota-credito");
const sri_soap_1 = require("./sri-soap");
const sri_signer_1 = require("./sri-signer");
const sri_ride_1 = require("./sri-ride");
const sri_email_1 = require("./sri-email");
...
let SriService = class SriService {
    ...
    async uploadCertificate(tenantId, fileBuffer, originalName, password) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configure primero los datos del emisor SRI');
        const tenantCertsDir = path.join(CERTS_DIR, tenantId);
        if (!fs.existsSync(tenantCertsDir))
            fs.mkdirSync(tenantCertsDir, { recursive: true });
        const sanitizedName = originalName.replace(/[^a-zA-Z0-9._-]/g, '_');
        const certPath = path.join(tenantCertsDir, sanitizedName);
        fs.writeFileSync(certPath, fileBuffer);
        const validation = (0, sri_signer_1.validateP12)(certPath, password);
        if (!validation.valid) {
            try {
                fs.unlinkSync(certPath);
            }
            catch { }
            throw new common_1.BadRequestException(`Certificado inválido: ${validation.error}`);
        }
        if (config.certP12Path && config.certP12Path !== certPath) {
            try {
                fs.unlinkSync(config.certP12Path);
            }
            catch { }
        }
    ...
    async deleteCertificate(tenantId) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        if (!config.certP12Path)
            throw new common_1.BadRequestException('No hay certificado configurado');
        if (config.certP12Path && fs.existsSync(config.certP12Path))
            fs.unlinkSync(config.certP12Path);
    ...
    async emitirFactura(tenantId, dto) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada. Configure primero los datos del emisor.');
        ...
        if (existingInv)
            throw new common_1.BadRequestException(`Esta orden ya tiene factura: ${existingInv.claveAcceso}`);
        ...
        const buyer = {
            ...
            direccion: dto.direccion || order.customer?.address || undefined,
            telefono: dto.telefono || order.customer?.phone || undefined,
        };
        const ivaInfo = sri_xml_1.IVA_RATES['15'] || sri_xml_1.IVA_RATES['12'] || { codigo: '4', tarifa: 15 };
        ...
        const formaPago = order.payments.map((p) => ({ codigo: sri_xml_1.PAYMENT_METHOD_MAP[p.method] || '01', total: parseFloat(p.amount.toString()) }));
        if (formaPago.length === 0)
            formaPago.push({ codigo: '01', total: importeTotal });
        const now = new Date();
        ...
        const claveAcceso = (0, sri_xml_1.generateClaveAcceso)(fechaEmision, '01', config.ruc, config.ambiente, config.establecimiento, config.puntoEmision, secuencial, codigoNumerico, config.tipoEmision);
        const sriConfig = this.buildSriConfig(config);
        const invoiceData = { config: sriConfig, secuencial, fechaEmision, buyer, details, totalSinImpuestos, totalDescuento: parseFloat(order.discountAmount.toString()), propina: 0, importeTotal, moneda: 'DOLAR', formaPago };
        const xmlGenerado = (0, sri_xml_1.generateFacturaXml)(invoiceData, claveAcceso);
        let xmlFirmado = null;
        let estado = 'generado';
        if (config.certP12Path && config.certP12Password) {
            try {
                xmlFirmado = (0, sri_signer_1.signXml)(xmlGenerado, config.certP12Path, decryptPassword(config.certP12Password));
                estado = 'firmado';
            }
            catch (e) {
                console.warn(`Auto-firma falló: ${e.message}`);
            }
        }
    ...
    async emitirNotaCredito(tenantId, dto) {
        ...
        if (!factura)
            throw new common_1.NotFoundException('Factura original no encontrada');
        if (factura.estado !== 'autorizado')
            throw new common_1.BadRequestException('Solo se pueden emitir notas de crédito sobre facturas autorizadas');
        ...
        if (existingNC)
            throw new common_1.BadRequestException(`Ya existe una nota de crédito para esta factura: ${existingNC.secuencial}`);
        ...
        if (dto.itemIds && dto.itemIds.length > 0) {
            orderItems = orderItems.filter((item) => dto.itemIds.includes(item.id));
        }
        if (orderItems.length === 0)
            throw new common_1.BadRequestException('No hay items para la nota de crédito');
        ...
        const fechaEmision = `${now.getDate().toString().padStart(2, '0')}/${(now.getMonth() + 1).toString().padStart(2, '0')}/${now.getFullYear()}`;
        const codigoNumerico = Math.floor(10000000 + Math.random() * 90000000).toString();
        const claveAcceso = (0, sri_xml_1.generateClaveAcceso)(fechaEmision, '04', config.ruc, config.ambiente, config.establecimiento, config.puntoEmision, secuencial, codigoNumerico, config.tipoEmision);
        ...
        const xmlGenerado = (0, sri_nota_credito_1.generateNotaCreditoXml)(ncData, claveAcceso);
        let xmlFirmado = null;
        let estado = 'generado';
        if (config.certP12Path && config.certP12Password) {
            try {
                ...
                console.warn(`Auto-firma NC falló: ${e.message}`);
            }
        }
    ...
    async anularComprobante(tenantId, invoiceId, dto) {
        ...
        throw new common_1.BadRequestException(`No se puede anular un comprobante en estado "${invoice.estado}"`);
    }
    async firmarFactura(tenantId, invoiceId) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        if (!config.certP12Path || !config.certP12Password)
            throw new common_1.BadRequestException('Certificado .p12 no configurado.');
        const invoice = await this.prisma.electronicInvoice.findFirst({ where: { id: invoiceId, tenantId } });
        if (!invoice)
            throw new common_1.NotFoundException('Comprobante no encontrado');
        if (!invoice.xmlGenerado)
            throw new common_1.BadRequestException('No hay XML generado para firmar');
        if (invoice.estado === 'autorizado')
            throw new common_1.BadRequestException('El comprobante ya está autorizado');
        try {
            const xmlFirmado = (0, sri_signer_1.signXml)(invoice.xmlGenerado, config.certP12Path, decryptPassword(config.certP12Password));
            ...
            throw new common_1.BadRequestException(`Error al firmar: ${e.message}`);
        }
    }
    async enviarAlSri(tenantId, invoiceId) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        const invoice = await this.prisma.electronicInvoice.findFirst({ where: { id: invoiceId, tenantId } });
        ...
        if (!xmlToSend)
            throw new common_1.BadRequestException('No hay XML para enviar');
        try {
            const recepcion = await (0, sri_soap_1.enviarComprobante)(xmlToSend, config.ambiente);
            ...
            return { estado: 'error', message: `Error de conexión con SRI: ${e.message}` };
        }
    }
    async consultarEstado(tenantId, invoiceId) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        const invoice = await this.prisma.electronicInvoice.findFirst({ where: { id: invoiceId, tenantId } });
        if (!invoice)
            throw new common_1.NotFoundException('Comprobante no encontrado');
        try {
            const result = await (0, sri_soap_1.consultarAutorizacion)(invoice.claveAcceso, config.ambiente);
            ...
            return { estado: 'error', message: `Error al consultar: ${e.message}` };
        }
    }
    async enviarEmail(tenantId, invoiceId, emailTo) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        if (!config.smtpHost || !config.smtpUser || !config.smtpPassword) {
            throw new common_1.BadRequestException('SMTP no configurado. Configure los datos SMTP en la pestaña Configuración.');
        }
        const invoice = await this.prisma.electronicInvoice.findFirst({
            where: { id: invoiceId, tenantId },
            include: { order: { include: { items: { include: { product: true }, where: { isVoid: false } }, payments: true } } },
        ...
        if (fs.existsSync(ridePath)) {
            ...
            try {
                ...
                rideBuffer = await (0, sri_ride_1.generateRidePdfBuffer)(rideData);
            }
            catch { }
        }
        const tipoDoc = (0, sri_email_1.getTipoDocLabel)(invoice.tipoComprobante);
        const numero = `${config.establecimiento}-${config.puntoEmision}-${invoice.secuencial.padStart(9, '0')}`;
        const result = await (0, sri_email_1.sendInvoiceEmail)(smtp, {
            to: emailTo,
    ...
    async generateRide(tenantId, invoiceId) {
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        const invoice = await this.prisma.electronicInvoice.findFirst({
        ...
        await (0, sri_ride_1.generateRidePdf)(rideData, tenantRidesDir);
    ...
    async downloadRide(tenantId, invoiceId) {
        const invoice = await this.prisma.electronicInvoice.findFirst({
            where: { id: invoiceId, tenantId },
            include: { order: { include: { items: { include: { product: true }, where: { isVoid: false } }, payments: true } } },
        });
        ...
        if (fs.existsSync(existingPath))
            return { buffer: fs.readFileSync(existingPath), filename: `RIDE-${invoice.secuencial}.pdf` };
        const config = await this.prisma.sriConfig.findUnique({ where: { tenantId } });
        if (!config)
            throw new common_1.BadRequestException('Configuración SRI no encontrada');
        const rideData = this.buildRideData(config, invoice);
        const buffer = await (0, sri_ride_1.generateRidePdfBuffer)(rideData);
        return { buffer, filename: `RIDE-${invoice.secuencial}.pdf` };
    }
    buildSriConfig(config) {
        return {
    ...
    buildRideData(config, invoice) {
        return {
            ...
            detalles: (invoice.order?.items || []).map((item) => ({
                codigoPrincipal: item.product?.sku || item.productId.substring(0, 25),
            ...
            pagos: (invoice.order?.payments || []).map((p) => ({ formaPago: sri_xml_1.PAYMENT_METHOD_MAP[p.method] || '01', total: parseFloat(p.amount.toString()) })),
            infoAdicional: [],
        };
    }

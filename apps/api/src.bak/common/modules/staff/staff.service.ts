...
export class StaffService {
  ...
  async getAll(tenantId: string) {
    const staff: any[] = await this.prisma.$queryRawUnsafe(`
  ...
  async getById(tenantId: string, userId: string) {
    ...
    // Current attendance
    const attendance: any[] = await this.prisma.$queryRawUnsafe(`
  ...
  async updateProfile(tenantId: string, userId: string, dto: UpdateStaffProfileDto) {
    const sets: string[] = [];
    if (dto.phone !== undefined) sets.push(`phone = ${dto.phone ? `'${dto.phone}'` : 'NULL'}`);
    if (dto.hireDate) sets.push(`hire_date = '${dto.hireDate}'::date`);
    if (dto.hourlyRate !== undefined) sets.push(`hourly_rate = ${dto.hourlyRate}`);
    if (dto.salary !== undefined) sets.push(`salary = ${dto.salary}`);
    if (dto.emergencyContact !== undefined) sets.push(`emergency_contact = ${dto.emergencyContact ? `'${dto.emergencyContact.replace(/'/g, "''")}'` : 'NULL'}`);
    if (dto.emergencyPhone !== undefined) sets.push(`emergency_phone = ${dto.emergencyPhone ? `'${dto.emergencyPhone}'` : 'NULL'}`);
    if (dto.taxId !== undefined) sets.push(`tax_id = ${dto.taxId ? `'${dto.taxId}'` : 'NULL'}`);
    if (dto.notes !== undefined) sets.push(`notes = ${dto.notes ? `'${dto.notes.replace(/'/g, "''")}'` : 'NULL'}`);

    if (sets.length === 0) return this.getById(tenantId, userId);
  ...
  async setSchedule(tenantId: string, dto: SetScheduleDto) {
  ...
  async getSchedule(tenantId: string, userId: string) {
  ...
  async getWeekSchedule(tenantId: string) {
    const rows: any[] = await this.prisma.$queryRawUnsafe(`
  ...
  async clockIn(tenantId: string, dto: ClockInDto) {
    ...
    `);
    if (active.length) throw new BadRequestException('Ya tiene registro de entrada activo');

    // Get today's schedule
    const dayOfWeek = new Date().getDay();
    const schedules: any[] = await this.prisma.$queryRawUnsafe(`
  ...
  async clockOut(tenantId: string, attendanceId: string, dto: ClockOutDto) {
    const rows: any[] = await this.prisma.$queryRawUnsafe(`
      SELECT * FROM staff_attendance
      WHERE id = '${attendanceId}'::uuid AND tenant_id = '${tenantId}'::uuid AND status = 'clocked_in'
    `);
    if (!rows.length) throw new NotFoundException('Registro de asistencia no encontrado');
    ...
    const updated: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT * FROM staff_attendance WHERE id = '${attendanceId}'::uuid`
    );
    return updated[0];
  }

  async getAttendance(tenantId: string, query: StaffQueryDto) {
    const { date, fromDate, toDate, userId, page = 1, limit = 50 } = query;
    const offset = (page - 1) * limit;

    let where = `WHERE sa.tenant_id = '${tenantId}'::uuid`;
    if (date) where += ` AND sa.clock_in::date = '${date}'::date`;
    if (fromDate) where += ` AND sa.clock_in::date >= '${fromDate}'::date`;
    if (toDate) where += ` AND sa.clock_in::date <= '${toDate}'::date`;
    if (userId) where += ` AND sa.user_id = '${userId}'::uuid`;
    ...
    const countResult: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT COUNT(*)::int as total FROM staff_attendance sa ${where}`
    );
  ...
  async getTodaySummary(tenantId: string) {
    ...
    const totalStaff: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT COUNT(*)::int as total FROM users WHERE tenant_id = '${tenantId}'::uuid AND is_active = true`
    );

    // Today's scheduled staff
    const dayOfWeek = new Date().getDay();
    const scheduled: any[] = await this.prisma.$queryRawUnsafe(`
  ...
  async getTimeOff(tenantId: string, query: StaffQueryDto) {
    const { userId, page = 1, limit = 50 } = query;
    let where = `WHERE t.tenant_id = '${tenantId}'::uuid`;
    if (userId) where += ` AND t.user_id = '${userId}'::uuid`;
  ...
  async reviewTimeOff(tenantId: string, id: string, status: 'approved' | 'rejected', approvedBy: string) {
    await this.prisma.$queryRawUnsafe(`
      UPDATE staff_time_off SET status = '${status}', approved_by = '${approvedBy}'::uuid, reviewed_at = NOW()
      WHERE id = '${id}'::uuid AND tenant_id = '${tenantId}'::uuid
    `);
    const rows: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT t.*, u.first_name, u.last_name FROM staff_time_off t JOIN users u ON u.id = t.user_id WHERE t.id = '${id}'::uuid`
    );
    return rows[0];
  }
  ...
  async getPayrollSummary(tenantId: string, fromDate: string, toDate: string) {
    const rows: any[] = await this.prisma.$queryRawUnsafe(`

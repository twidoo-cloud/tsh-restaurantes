...
const del = async <T,>(p: string): Promise<T> => { const r = await fetch(`${API}${p}`, { method: 'DELETE', headers: hdrs() }); if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Error'); return r.json(); };

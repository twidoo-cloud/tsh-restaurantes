...
import { ArrowLeft, Flame, Clock, Check, ChefHat, Bell, Utensils, RefreshCw, AlertTriangle, Wifi, WifiOff, Volume2, VolumeX } from 'lucide-react';
...
function getUrgencyColor(seconds: number): string {
  if (seconds > 900) return 'text-red-600'; // >15min
  if (seconds > 600) return 'text-orange-500'; // >10min
  if (seconds > 300) return 'text-yellow-600'; // >5min
  return 'text-gray-500';
}

// Extend api with request method if not available
...
export default function KitchenPage() {
  ...
  // Periodic urgency alert for orders older than 10 minutes
  useEffect(() => {
    ...
    const urgencyCheck = setInterval(() => {
      ...
      if (hasUrgent) {
        playUrgentSound();
        showToast('⚠️ Hay órdenes con más de 10 min', 'urgent');
      }
  ...
  useEffect(() => {
    ...
    loadOrders();
    // Fallback polling at 30s (instead of 5s) since we have WebSocket now
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-900" onClick={() => initAudio()}>
      {/* Header — dark theme for kitchen */}
      <header className="flex h-14 items-center justify-between bg-gray-800 px-4 text-white border-b border-gray-700">
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/pos')} className="rounded-lg p-1.5 transition hover:bg-gray-700">
            <ArrowLeft size={20} />
          </button>
          <ChefHat size={22} className="text-orange-400" />
          <span className="text-lg font-bold">Kitchen Display</span>
          <span className="text-sm text-gray-400">{store.tenant?.name}</span>
        </div>

        <div className="flex items-center gap-4">
          {/* Stats */}
          <div className="flex items-center gap-3 text-sm">
            <span className="flex items-center gap-1.5 rounded-full bg-yellow-900/50 px-3 py-1 text-yellow-300">
              <Clock size={14} /> {totalPending} pendientes
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-blue-900/50 px-3 py-1 text-blue-300">
              <Flame size={14} /> {totalPreparing} preparando
            </span>
            <span className="flex items-center gap-1.5 rounded-full bg-green-900/50 px-3 py-1 text-green-300">
              <Check size={14} /> {readyOrders.length} listos
            </span>
          </div>
          ...
          <button onClick={() => { initAudio(); setSoundEnabled(!soundEnabled); }}
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium transition ${
              soundEnabled ? 'bg-green-700 text-green-100' : 'bg-gray-700 text-gray-400'
            }`}>
          ...
          {/* Refresh */}
          <button onClick={loadOrders} className="rounded-lg p-2 text-gray-400 transition hover:bg-gray-700 hover:text-white">
            <RefreshCw size={18} />
          </button>
      ...
      {/* Station filter tabs */}
      <div className="flex gap-2 bg-gray-800 px-4 py-2 border-b border-gray-700">
        {STATIONS.map((s) => (
          <button key={s.id || 'all'} onClick={() => setActiveStation(s.id)}
            ...
            {s.icon ? <s.icon size={16} /> : <span>{s.emoji}</span>}
            {s.label}
          </button>
        ))}
      </div>
            ...
            {readyOrders.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {readyOrders.map((order: any) => (
                  <div key={order.orderId} className="rounded-xl border-2 border-green-500 bg-green-950/50 p-4 animate-pulse-slow">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <span className="text-lg font-bold text-green-300">{order.orderNumber}</span>
                        {order.tableNumber && (
                          <span className="ml-2 rounded bg-green-800 px-2 py-0.5 text-xs font-medium text-green-200">
                            Mesa {order.tableNumber}
                          </span>
                        )}
                      </div>
                      <span className="text-sm text-green-400">
                        {formatTime(order.waitingSeconds)} esperando
                      </span>
                    </div>
                    ...
                    <button onClick={() => { order.items.forEach((i: any) => handleMarkDelivered(i.id)); }}
                      className="w-full rounded-lg bg-green-600 py-2 text-sm font-semibold text-white transition hover:bg-green-500">
                      ✓ Entregado
                    </button>
                  </div>
                ))}
            ...
            {filteredOrders.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredOrders.map((order: any) => {
                  ...
                  const isUrgent = maxElapsed > 600; // >10min
                  const allPreparing = order.items.every((i: any) => i.status === 'preparing');

                  return (
                    <div key={order.orderId}
                      className={`rounded-xl border-2 bg-gray-800 p-4 transition ${
                      ...
                      <div className="space-y-2 mb-3">
                        {order.items.map((item: any) => {
                          ...
                          return (
                            <div key={item.id}
                              className={`rounded-lg border p-2.5 ${st.bg} cursor-pointer transition hover:opacity-80`}
                              onClick={() => {
      ...
      {/* Toast notification */}
      {toast && (
        <div className={`fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-bounce rounded-xl px-6 py-3 shadow-2xl text-white font-bold text-lg ${

...
const get = async (path: string) => { const r = await fetch(`${API}${path}`, { headers: headers() }); if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Error'); return r.json(); };
const post = async (path: string, body: any) => { const r = await fetch(`${API}${path}`, { method: 'POST', headers: headers(), body: JSON.stringify(body) }); if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Error'); return r.json(); };
const put = async (path: string, body: any) => { const r = await fetch(`${API}${path}`, { method: 'PUT', headers: headers(), body: JSON.stringify(body) }); if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Error'); return r.json(); };
const del = async (path: string) => { const r = await fetch(`${API}${path}`, { method: 'DELETE', headers: headers() }); if (!r.ok) throw new Error((await r.json().catch(() => ({}))).message || 'Error'); return r.json(); };

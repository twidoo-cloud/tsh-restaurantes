...
export default function ShiftsPage() {
  ...
  // ═══ Cash Register CRUD Handlers ═══

  const handleCreateRegister = async () => {
    if (!newRegisterName.trim()) return;
    setActionLoading(true);
    try {
      await apiRequest('/shifts/cash-registers', {
        method: 'POST',
  ...
  const handleUpdateRegister = async () => {
    if (!editingRegister || !editName.trim()) return;
    setActionLoading(true);
    try {
      await apiRequest(`/shifts/cash-registers/${editingRegister.id}`, {
        method: 'PUT',
        body: JSON.stringify({ name: editName.trim() }),
      });
      setEditingRegister(null); setEditName('');
      await loadData();
    } catch (e: any) { alert(e.message); }
    setActionLoading(false);
  };
  ...
  const handleDeleteRegister = async (id: string, name: string) => {
    if (!confirm(`¿Eliminar/desactivar la caja "${name}"?`)) return;
    try {
      await apiRequest(`/shifts/cash-registers/${id}`, { method: 'DELETE' });
      await loadData();
    } catch (e: any) { alert(e.message); }
  };
  ...
  const handleToggleActive = async (id: string, currentActive: boolean) => {
    try {
      await apiRequest(`/shifts/cash-registers/${id}`, {
        method: 'PUT',
  ...
  return (
    ...
    <div className="flex h-full flex-col bg-gray-100">
        <div className="flex items-center justify-between bg-white border-b px-4 py-2.5 shrink-0">
          <h1 className="text-lg font-bold text-gray-900">Turnos / Caja</h1>
          <div className="flex items-center gap-2">
            <button onClick={() => setShowManageDialog(true)} className="flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-1.5 text-sm text-blue-700 hover:bg-blue-100">
              <Settings size={16} /> Administrar Cajas
            </button>
            <button onClick={() => { loadHistory(); setShowHistoryDialog(true); }} className="flex items-center gap-1.5 rounded-lg bg-gray-100 px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-200">
        ...
        {activeShift ? (
          ...
          <div className="mx-auto max-w-2xl">
            <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm text-gray-500">Abre un turno para comenzar a registrar ventas.</p>
              <div className="mt-6 space-y-3">
                {cashRegisters.filter((cr: any) => cr.is_active).length === 0 ? (
                  <div className="rounded-xl border-2 border-dashed border-gray-300 p-6">
                    <p className="text-sm text-gray-500 mb-3">No hay cajas registradoras en esta sucursal.</p>
                    <button onClick={() => setShowManageDialog(true)}
                      className="flex items-center gap-1.5 mx-auto rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
                      <Plus size={16} /> Crear Caja Registradora
                    </button>
                  </div>
                ) : (
                  cashRegisters.filter((cr: any) => cr.is_active).map((cr: any) => (
                    <div key={cr.id} className={`flex items-center justify-between rounded-xl border-2 p-4 transition ${cr.has_open_shift > 0 ? 'border-blue-300 bg-blue-50' : 'border-gray-200 hover:border-green-300'}`}>
                      <div className="flex items-center gap-3">
                        <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${cr.has_open_shift > 0 ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'}`}><Receipt size={20} /></div>
                        <div className="text-left">
                          <p className="font-medium text-gray-900">{cr.name}</p>
                          {cr.has_open_shift > 0 ? <p className="text-xs text-blue-600">Turno abierto por {cr.opened_by_name}</p> : <p className="text-xs text-gray-500">Disponible</p>}
                        </div>
                      </div>
                      {cr.has_open_shift === 0 && (
      ...
      {/* ═══ MANAGE CASH REGISTERS DIALOG ═══ */}
      {showManageDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowManageDialog(false)}>
          <div className="w-full max-w-lg max-h-[80vh] rounded-2xl bg-white shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h3 className="text-lg font-bold text-gray-900">Administrar Cajas Registradoras</h3>
              <button onClick={() => setShowManageDialog(false)} className="rounded-lg p-1 text-gray-400 hover:bg-gray-100"><X size={18} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400/20"
                  onKeyDown={e => { if (e.key === 'Enter') handleCreateRegister(); }}
                />
                <button
                  onClick={handleCreateRegister}
                  disabled={actionLoading || !newRegisterName.trim()}
              ...
              <div className="space-y-2">
                {cashRegisters.length === 0 ? (
                  <p className="text-center text-sm text-gray-400 py-4">No hay cajas registradoras</p>
                ) : (
                  cashRegisters.map((cr: any) => (
                    <div key={cr.id} className={`flex items-center justify-between rounded-xl border p-3 ${cr.is_active ? 'bg-white' : 'bg-gray-50 opacity-60'}`}>
                      {editingRegister?.id === cr.id ? (
                        <div className="flex flex-1 gap-2">
                          <input
                            ...
                            onKeyDown={e => { if (e.key === 'Enter') handleUpdateRegister(); if (e.key === 'Escape') setEditingRegister(null); }}
                            autoFocus
                          />
                          <button onClick={handleUpdateRegister} className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-blue-700">Guardar</button>
                          <button onClick={() => setEditingRegister(null)} className="rounded-lg border px-3 py-1.5 text-xs text-gray-600 hover:bg-gray-50">Cancelar</button>
                        </div>
                      ) : (
                        <>
                          <div className="flex items-center gap-3">
                            <div className={`flex h-9 w-9 items-center justify-center rounded-lg ${cr.is_active ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                              <Receipt size={18} />
                            </div>
                            <div>
                              <p className="font-medium text-gray-900 text-sm">{cr.name}</p>
                              <p className="text-xs text-gray-400">
                                {cr.is_active ? 'Activa' : 'Inactiva'}
                                {cr.has_open_shift > 0 && ' — Turno abierto'}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            <button
                              onClick={() => { setEditingRegister(cr); setEditName(cr.name); }}
                            ...
                            <button
                              onClick={() => handleToggleActive(cr.id, cr.is_active)}
                              className={`rounded-lg p-1.5 hover:bg-gray-100 ${cr.is_active ? 'text-gray-400 hover:text-amber-600' : 'text-gray-400 hover:text-green-600'}`}
                              title={cr.is_active ? 'Desactivar' : 'Activar'}
                            >
                              {cr.is_active ? <Lock size={14} /> : <Unlock size={14} />}
                            </button>
                            {cr.has_open_shift === 0 && (
                              <button
                                onClick={() => handleDeleteRegister(cr.id, cr.name)}

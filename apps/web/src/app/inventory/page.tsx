...
export default function InventoryPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [stock, movs, alertsData, sum, supps] = await Promise.all([
      ...
      setSummary(sum);
      setSuppliers(Array.isArray(supps) ? supps : (supps as any).data || []);
      setLoading(false);
    } catch (e) {
      console.error(e);
      setLoading(false);
    ...
    <div className="flex h-full flex-col bg-gray-50">
      {/* Header */}
        ...
        <div className="flex items-center justify-between bg-white border-b px-4 py-2.5 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => {
              const data = tab === 'stock' ? stockData : movements.data;
              if (!data?.length) return;
              let csv = '';
              if (tab === 'stock') {
                csv = 'Producto,SKU,Categoría,Stock Actual,Stock Mín,Unidad,Costo,Valor Total,Estado\n';
                csv += data.map((p: any) => `"${p.name}","${p.sku || ''}","${p.category_name || ''}",${p.current_stock},${p.min_stock},"${p.unit}",${p.cost || 0},${((p.current_stock || 0) * (p.cost || 0)).toFixed(2)},"${p.stock_status || ''}"`).join('\n');
              } else {
                csv = 'Fecha,Producto,Tipo,Cantidad,Referencia,Notas,Usuario\n';
                csv += data.map((m: any) => `"${m.created_at || m.createdAt}","${m.product_name || m.productName || ''}","${m.type}",${m.quantity},"${m.reference || ''}","${(m.notes || '').replace(/"/g, '""')}","${m.user_name || m.userName || ''}"`).join('\n');
              }
              const blob = new Blob([csv], { type: 'text/csv' });
              const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
              a.download = `inventario-${tab}-${new Date().toISOString().slice(0,10)}.csv`; a.click();
            }} className="flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium text-gray-600 hover:bg-gray-50">
              <ArrowUpDown size={14} /> CSV
            </button>
      ...
      {summary && (
        <div className="grid grid-cols-5 gap-3 px-4 py-3 bg-white border-b">
          ...
          <SummaryCard label="Valor Total" value={formatMoney(summary.totalStockValue)} icon={DollarSign} color="text-green-600" bg="bg-green-50" />
                  ...
                  {stockData.map(p => {
                    ...
                    return (
                      <tr key={p.id} className={`hover:bg-gray-50 ${p.stockStatus === 'out_of_stock' ? 'bg-red-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-right font-medium text-gray-700">{formatMoney(p.stockValue)}</td>
                        <td className="px-4 py-3 text-center">
      ...
      {showMovementForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => { setShowMovementForm(false); resetForm(); }}>
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {/* Supplier (for purchase and return) */}
            {(movementType === 'purchase' || movementType === 'return') && (
              <div className="mb-4">
                ...
                <select value={selectedSupplier} onChange={e => setSelectedSupplier(e.target.value)}
                  ...
                  {suppliers.map((s: any) => (
                    <option key={s.id} value={s.id}>{s.name}{s.contact_name || s.contactName ? ` (${s.contact_name || s.contactName})` : ''}</option>
                  ))}
                </select>
              </div>
            )}

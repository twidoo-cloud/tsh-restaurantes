...
class RecordMovementDto {
  ...
  @ApiPropertyOptional({ example: 15.50 }) @IsOptional() @IsNumber() unitCost?: number;
...
class BulkAdjustmentItem {
  ...
  @IsNumber() newStock: number;
  @IsOptional() @IsString() notes?: string;
}

class BulkAdjustmentDto {
  @IsArray() @ValidateNested({ each: true }) @Type(() => BulkAdjustmentItem)
...
export class InventoryController {
  ...
  async recordMovement(
    ...
    return this.inventoryService.recordMovement(tenantId, userId, dto as any, branchId);
  }

...
export default function SriPage() {
  ...
  const uploadCertificate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/sri/certificate`, { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : {}, body: fd });
                ...
                <div>
                  ...
                  <select value={emitForm.orderId} onChange={e => setEmitForm({ ...emitForm, orderId: e.target.value })} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-900">
                    ...
                    {completedOrders.map((o: any) => {
                      const num = o.order_number || o.orderNumber || o.id?.substring(0, 8) || '?';
                      ...
                      const typeMap: Record<string, string> = { dine_in: tableNum ? `Mesa ${tableNum}` : 'Mesa', takeout: 'Llevar', delivery: 'Delivery', sale: 'Mostrador' };
                      const typeLabel = typeMap[o.type] || '';
                      const date = o.created_at || o.createdAt;
                      const timeStr = date ? new Date(date).toLocaleTimeString('es-EC', { hour: '2-digit', minute: '2-digit' }) : '';
                      const server = o.server_name || '';
                      const items = o.items_count ? `${o.items_count} items` : '';
                      const detail = [typeLabel, server, items, timeStr].filter(Boolean).join(' · ');
                      return (
                        <option key={o.id} value={o.id}>
                          #{num} — {formatMoney(parseFloat(o.total))}{detail ? ` (${detail})` : ''}
                        </option>
                      );
                    })}
                  </select>
